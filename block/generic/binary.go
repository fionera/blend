...
func Read(r io.Reader, order binary.ByteOrder, ptrSize int, data any) error {
	...
	d := &decoder{order: order, buf: make([]byte, size), ptrSize: ptrSize}
...
func Write(w io.Writer, order binary.ByteOrder, ptrSize int, data any) error {
	// Fast path for basic types and slices.
	if n := intDataSize(data); n != 0 {
		bs := make([]byte, n)
		switch v := data.(type) {
	...
	buf := make([]byte, size)
	e := &encoder{order: order, buf: buf, ptrSize: ptrSize}
	e.value(v)
	_, err := w.Write(buf)
	return err
}
...
func (d *decoder) value(v reflect.Value) {
	...
	case reflect.Struct:
		...
		l := v.NumField()
		for i := 0; i < l; i++ {
			if t.Field(i).Tag.Get("bin") == "ptrSize" {
				var p uint64
				if d.ptrSize == 4 {
					p = uint64(d.uint32())
				} else {
				...
				v.Field(0).Set(reflect.ValueOf(p))
				continue
			}
...
func (e *encoder) value(v reflect.Value) {
	...
	case reflect.Struct:
		...
		for i := 0; i < l; i++ {
			if t.Field(i).Tag.Get("bin") == "ptrSize" {
				if e.ptrSize == 4 {
					e.uint32(uint32(v.Field(0).Uint()))
				} else {
					e.uint64(v.Field(0).Uint())
				}
				continue
			}
